...
def get_wiki_from_file():
    wiki_records = {}
    book = xlrd.open_workbook('../twitter/politycy.xls')
    ...
    try:
        ...
        for name, wiki_name in resources.items():
            ...
            if not production:
                cursor.execute(
                    f"""
                    SELECT politicians.id, pw.summary
                    FROM politicians
                    LEFT JOIN politicians_wiki pw on politicians.id = pw.politician
                    WHERE name='{name.split()[1]}' AND surname ='{name.split()[0]}';""")
                wiki_record = cursor.fetchone()


                if wiki_record and not wiki_record[1]:
                    cursor.execute(
                        f"INSERT INTO politicians_wiki (url, politician, summary) VALUES('{wiki_name}','{wiki_record[0]}', '{summary}');")
                    connection.commit()
                elif wiki_record and wiki_record[1]:
                    cursor.execute(
                        f"UPDATE politicians_wiki SET summary = '{summary}' WHERE id = {wiki_record[0]};")
                    connection.commit()
            else:
                cursor.execute(f"UPDATE politicians_wiki SET summary = '{summary}' WHERE id = {name};")
                connection.commit()
